...
class ReservationJdbcRepositoryTest {
    ...
    void testSaveAndFindById() {
        ...
        Reservation reservation = new Reservation("R1", date, "A", "B", List.of(seat1, seat2), 100, ReservationStatus.HELD);
        ...
        // Seats should now be marked reserved
    ...
    void testFindAvailableSeats() {
        // Reserve S1
        LocalDate date = LocalDate.now();
        Reservation reservation = new Reservation("R2", date, "A", "B", List.of(new Seat("S1", false)), 50, ReservationStatus.HELD);
        repository.save(reservation, 50);

        List<Seat> available = repository.findAvailableSeats(date, "A", "B");
    ...
    void testFindBookedSeats() {
        LocalDate date = LocalDate.now();
        Reservation reservation = new Reservation("R3", date, "A", "C", List.of(new Seat("S2", false)), 50, ReservationStatus.HELD);
        repository.save(reservation, 50);
    ...
    void testUpdateReservationStatus() {
        LocalDate date = LocalDate.now();
        Reservation reservation = new Reservation("R4", date, "B", "C", List.of(new Seat("S3", false)), 50, ReservationStatus.HELD);
        repository.save(reservation, 50);
